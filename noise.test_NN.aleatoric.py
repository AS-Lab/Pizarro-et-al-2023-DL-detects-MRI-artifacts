...
def get_subj_data(p,mc=1,input_size=(1,256,256,64,1)):
...
def run_NN_test_mc(models_fn,exp_dirs,XV_sets,mc_runs,longitudinal='False',cln_prct='clean_percent_098'):
    experiment = '/'.join(exp_dirs)
    paths_dir = '/trials/data/rpizarro/noise/XValidFns/multiple_artifact/clean_percent_098/XV0'
    if cln_prct=='clean_percent_050':
        # one time inference on 50% validation set using the 98% models
        paths_dir = '/trials/data/rpizarro/noise/XValidFns/multiple_artifact/clean_percent_050/XV0_used'

    if longitudinal=='True':
        # one time inference on 50% validation set using the 98% models
        paths_dir = '/trials/data/rpizarro/noise/XValidFns/multiple_artifact/clean_percent_050/XV0_used'
    # paths_dir = os.path.join('/data/datasets/shared/rpizarro/noise/weights','{}'.format(experiment))
    for model_fn in models_fn:
        ...
        custom_objects = None
        model_weights = load_model(model_fn,custom_objects)
        model = get_new_model()
        ...
        for XV_set in XV_sets:
            ...
            save_dir = '/trials/data/rpizarro/noise/prediction/{0}/{1}/non_MC'.format(experiment,XV_set)
            if longitudinal=='True':
                save_dir = '/trials/data/rpizarro/noise/prediction/{0}/longitudinal/ep{1:04d}/{2}/'.format(experiment,ep,XV_set)
            ...
            set_fn = os.path.join(paths_dir,'{0}.art123.csv'.format(XV_set))
